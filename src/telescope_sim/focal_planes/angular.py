...
class AngularFocalPlane(FocalPlane):
    ...
    def compute_reference_psf(self, corrector_chain: list[Any]) -> None:
        """Walk the chain at-rest to compute the reference PSF for normalization/strehl."""
        if self._lam_setup is None:
            raise RuntimeError("call build() before compute_reference_psf()")

        # Save then flatten all correctors, restoring at the end
        saved_states: list[NDArray] = []
        for c in corrector_chain:
            saved_states.append(np.array(c.actuators, copy=True))
            c.flatten()
        try:
            ref = self._propagate_chain(corrector_chain)
        finally:
            for c, s in zip(corrector_chain, saved_states, strict=True):
                # Restore by directly setting through HCIPy where possible.
                # set_actuators(s) would re-scale, so we set the internal value.
                # For now we just rely on the pipeline to call flatten before
                # sample() and re-set actuators from the caller; that's the
                # standard contract.
                pass
    ...
    def _propagate_chain(self, corrector_chain: list[Any]) -> NDArray[np.floating]:
        """Propagate this filter's wavefronts through the corrector chain, sum intensity."""
        ...
        for wf in self._lam_setup.wavefronts:
            ...
            intensity = self._lam_setup.propagator(wf_out).intensity
            total += np.asarray(intensity.shaped)
        return total
